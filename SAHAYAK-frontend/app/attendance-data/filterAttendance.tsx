    ...
    {item.overtimeEntry && (
      <View style={styles.timeSection}>
        ...
        <View style={styles.timeRow}>
          ...
          <Text style={styles.timeLabel}>Out: {item.overtimeLeaving ?? ''}</Text>
        </View>
      </View>
    )}
  </View>
...
export default function App() {
  ...
  useEffect(() => {
    const fetchTodayData = async () => {
      ...
      const data = await fetchAttendanceDataByDate({day: day, month: month, year: year})
  ...
  const handleDownloadPdf = async () => {
    await generateAttendancePDF(workers, `${selectedDate.getDate()}/${selectedDate.getMonth()}/${selectedDate.getFullYear()}`)
    alert('Downloaded');
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Modal visible={showCalendar} transparent animationType="fade" onRequestClose={() => setShowCalendar(false)}>
        ...
        <View style={{
          ...
          <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 8, color: '#222' }}>
            Select date
          </Text>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 10 }}>
            ...
            >
              <Text style={{ color: '#4A90E2', fontWeight: '600' }}>Cancel</Text>
            </TouchableOpacity>
